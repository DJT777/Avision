...
from tracklite.deep_sort import DeepSort
...
def convert_tlbr_tlwh(bboxes):
    ...
    numpy_array_of_array = np.array([np.array(x_array) for x_array in list_of_boxes])
    return numpy_array_of_array
...
class TrtYOLO(object):
    ...
    def __init__(self, model, cfg, category_num=80, letter_box=False, cuda_ctx=None):
        ...
        #DEEPSORT CFG
        self.cfg = cfg
        self.deep_sort = build_tracker(cfg, use_cuda=True)
    ...
    def detect(self, img, conf_th=0.3, letter_box=None):
        ...
        outputs = self.deep_sort.update(boxes, scores, img)
