...
from tracklite.deep_sort import build_tracker
from tracklite.utils.draw import *
...
#USING X CENTER Y CENTER WIDTH AND HEIGHT
def xxyy_to_xywh(bboxes):
    xywh_list = []
    for item in bboxes:
        new_box_list = []
        xc = (item[1]+item[3])/2
        ...
        w = (item[3]-item[1])
        h = (item[4]-item[2])
        new_box_list.append(xc)
        new_box_list.append(yc)
        new_box_list.append(w)
        new_box_list.append(h)
        xywh_list.append(new_box_list)
    return xywh_list
...
class TrtYOLO(object):
    """TrtYOLO class encapsulates things neeuseded to run TRT YOLO."""
    ...
    def __init__(self, model, cfg, category_num=80, letter_box=False, cuda_ctx=None):
    ...
    def detect(self, img, conf_th=0.3, letter_box=None):
        ...
        xywh_boxes = xxyy_to_xywh(boxes)

        tracker_outputs = self.deepsort.update(xywh_boxes, scores, img)
