...
def loop_and_detect(cam, trt_yolo, conf_th, vis):
    ...
    while True:
        ...
        mutex.release()
        t = Thread(target=mysql_insert, args=(mySQLConnection, outputs, scores, today_formatted, time_formatted, image_blob))
        t.start()
        mutex.acquire()
...
def mysql_insert(mySQLConnection, bbox_xyxy, scores, today, time, img_blob):
    mutex.acquire()
    ...
    if len(copy_bbox) > 0:
        ...
        for box, identity, score in zip(bbox_xyxy, identities, copy_scores):
            if score > 0 :
                ...
                mySQLCursor.execute(sql_op, (copy_today, copy_time, confidence, identity, x1, y1, x2, y2, img_blob))
    mySQLCursor.close()
